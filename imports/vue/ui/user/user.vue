<template>
  <v-row justify="center">
    ...
    <v-dialog v-model="dialog" persistent max-width="1400px" :fullscreen="$vuetify.breakpoint.mobile">
      <v-form
          ...
          <v-card-text>
            <v-container>
              <v-row>
                ...
                <v-col cols="12" md="3" sm="3" xs="3">
                  <v-select
                      v-model="user.defaultModule"
                      :items="defaultModuleOptionList"
                      :dense="dense"

                      chips
                      :label="$t('defaultModule')"
                      small-chips
                      outlined
                  ></v-select>
                </v-col>
...
export default {
  ...
  watch: {
    ...
    "user.module"(val) {
      let vm = this;
      val.forEach((obj) => {
        let d = vm.modulesOptionList.find(o => o === obj);
        if (d) {
          vm.defaultModuleOptionList.push(d);
        }
      })
    }
  ...
  created() {
    ...
    let branchDoc = vm.$store.state.branchDoc;
    vm.modulesOptionList = Constants.modulesOption
    if (branchDoc && branchDoc.name !== undefined) {
      vm.modulesOptionList = branchDoc.module;
    }
