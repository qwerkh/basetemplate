          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('balanceSheetTitle') }}
                  <v-spacer></v-spacer>
           ...
           style="align-items: center; margin-top: 0px !important;margin-bottom: 5px !important;border-top:1px solid black !important;margin-left: 25% !important;margin-right: 25% !important;">
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;padding: 0px !important;">
                        <span
                            style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{ $t('balanceSheetTitle') }}</span><br>
                </v-col>
...
export default {
  ...
  watch: {
    ...
    searchExchange(val) {
      this.fetchExchangeOptionList(val)
    },
    "dataHtml"(val) {
      let vm = this;
      if (val) {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("Balancesheet");
      workbook.Sheets["Balancesheet"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "Balancesheet " + moment().format("YYYYMMDD") + ".xlsx");
    },
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('acc_fetchBalanceSheetReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
