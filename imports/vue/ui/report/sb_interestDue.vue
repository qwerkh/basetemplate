          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('interestDueTitle') }}
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("InterestDue");
      workbook.Sheets["InterestDue"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "InterestDue " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        this.dataHtml = "";
        Meteor.call('sb_fetchInterestDueReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
