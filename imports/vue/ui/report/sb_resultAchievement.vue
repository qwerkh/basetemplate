<!--suppress ALL -->
<template>
  <div class="resultAchievement-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('resultAchievementTitle') }}
                  <v-spacer></v-spacer>
          ...
          style="align-items: center; margin-top: 0px !important;margin-bottom: 5px !important;border-top:1px solid black !important;margin-left: 25% !important;margin-right: 25% !important;">
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;padding: 0px !important;">
                        <span
                            style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{ $t('resultAchievementTitle') }}</span><br>
                </v-col>
                ...
                <thead style="margin-top: 5px">
                    <tr>
                       <th>{{ $t('no') }}</th>
                       <th>{{ $t('creditOfficer') }}</th>
                       <th>{{ $t('currency') }}</th>
                       <th>{{ $t('phoneNumber') }}</th>
                                               <th>{{ $t('amount') }}</th>
                                               <th>{{ $t('achieveMonthly') }}</th>
                                               <th>{{ $t('achieveYearly') }}</th>
                                               <th>{{ $t('planAmountInYear') }}</th>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("ResultAchievement");
      workbook.Sheets["ResultAchievement"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "ResultAchievement " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('sb_fetchResultAchievementReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
