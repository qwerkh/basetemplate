<!--suppress ALL -->
<template>
  <div class="productActivity-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('productActivityTitle') }}
                  <v-spacer></v-spacer>
          ...
          style="align-items: center; margin-top: 0px !important;margin-bottom: 5px !important;border-top:1px solid black !important;margin-left: 25% !important;margin-right: 25% !important;">
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;padding: 0px !important;">
                        <span
                            style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{ $t('productActivityTitle') }}</span><br>
                </v-col>
                ...
                <thead style="margin-top: 5px">
                    <tr>
                       ...
                       <th>{{ $t('staffName') }}</th>
                        <th>{{ $t('currency') }}</th>
                       <th>{{ $t('loanAmount') }}</th>
                       <th>{{ $t('newLoanAmount') }}</th>
                       <th>{{ $t('newClient') }}</th>
                       <th>{{ $t('oldClient') }}</th>
                       <th>{{ $t('totalClient') }}</th>
                       <th>{{ $t('fee') }}</th>
                       <th>{{ $t('colPrinciple') }}</th>
                       <th>{{ $t('colInterest') }}</th>
                       <th v-show="isOperationFee">{{ $t('colOperationFee') }}</th>

                       <th>{{ $t('colPenalty') }}</th>
                       <th>{{ $t('totalCollect') }}</th>
                       <th>{{ $t('loanOutStanding') }}</th>
                       <th>{{ $t('arrearsPrin') }}</th>
                       <th>{{ $t('arrearsInt') }}</th>
                        <th v-show="isOperationFee">{{ $t('arrearsOperationFee') }}</th>
                       <th>{{ $t('par') }}</th>
                       <th>{{ $t('parNBC') }}</th>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("ProductActivity");
      workbook.Sheets["ProductActivity"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "ProductActivity " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('sb_fetchProductActivityReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
