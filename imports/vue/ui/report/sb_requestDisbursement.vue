<!--suppress ALL -->
<template>
  <div class="requestDisbursement-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('requestDisbursementTitle') }}
                  <v-spacer></v-spacer>
...
export default {
  name: "RequestDisbursementReport",
  meteor: {},
  mounted() {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("RequestDisbursement");
      workbook.Sheets["RequestDisbursement"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "RequestDisbursement " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        this.dataHtml = "";
        Meteor.call('sb_fetchRequestDisbursementReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
