          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('openAccountTitle') }}
                  <v-spacer></v-spacer>
          ...
          style="align-items: center; margin-top: 0px !important;margin-bottom: 5px !important;border-top:1px solid black !important;margin-left: 25% !important;margin-right: 25% !important;">
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;padding: 0px !important;">
                        <span style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{ $t('openAccountTitle') }}</span><br>
                </v-col>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("OpenAccount");
      workbook.Sheets["OpenAccount"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "OpenAccount " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('sb_fetchOpenAccountReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
