<!--suppress ALL -->
<template>
  <div class="loanOutstanding-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('loanOutstandingTitle') }}
                  <v-spacer></v-spacer>
                ...
                <v-card-text>
                  <v-row>
                    ...
                    <v-col cols="12" sm="4" md="4">
                      <v-switch v-model="params.minusPrepay" class="mx-2"
                                :label="$t('minusPrepay')"
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <thead style="margin-top: 5px">
                    <tr>
                       ...
                       <th>{{ $t('installmentRemain') }}</th>
                       <th>{{ $t('interestRate') }}</th>

                       <th>{{ $t('principleAmount') }}</th>
                       <th v-if="isShowTotal">{{ $t('projectInterest') }}</th>
                       <th v-if="isShowTotal">{{ $t('total') }}</th>
                       <th>{{ $t('principlePaid') }}</th>
                       <th>{{ $t('interestPaid') }}</th>
                       <th v-if="isShowTotal">{{ $t('totalPaid') }}</th>
                       <th v-show="isOperationFee">{{ $t('operationFeePaid') }}</th>
                       <th>{{ $t('adminFeePaid') }}</th>
                       <th v-if="!isNotPenalty">{{ $t('penaltyPaid') }}</th>
                       <th>{{ $t('outstanding') }}</th>
                       <th v-if="isShowTotal">{{ $t('interestRemain') }}</th>
                        <th>{{ $t('currency') }}</th>
                       <th>{{ $t('accType') }}</th>
                                             <th>{{ $t('phoneNumber') }}</th>
...
export default {
  ...
  created() {
    ...
    this.isShowTotal = loanConfig && loanConfig.isShowTotal || false;
    this.isNotPenalty = loanConfig && loanConfig.isNotPenalty || false;
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("LoanOutstanding");
      workbook.Sheets["LoanOutstanding"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "LoanOutstanding " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        this.dataHtml = "";
        Meteor.call('sb_fetchLoanOutstandingReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
