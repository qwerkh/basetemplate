<!--suppress ALL -->
<template>
  <div class="requestOpenAccount-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('requestOpenAccountTitle') }}
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("RequestOpenAccount");
      workbook.Sheets["RequestOpenAccount"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "RequestOpenAccount " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        this.dataHtml = "";
        Meteor.call('sb_fetchRequestOpenAccountReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
