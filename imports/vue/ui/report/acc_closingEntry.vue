          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('closingEntryTitle') }}
                  <v-spacer></v-spacer>
...
export default {
  ...
  created() {
    ...
    let closingEntryId = vm.$route.params.closingEntryId;
    if (closingEntryId) {
      this.params.closingEntryId = closingEntryId;
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("ClosingEntry");
      workbook.Sheets["ClosingEntry"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "ClosingEntry " + moment().format("YYYYMMDD") + ".xlsx");
    },
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('acc_fetchClosingEntryReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
