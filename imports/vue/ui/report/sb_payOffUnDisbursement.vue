<!--suppress ALL -->
<template>
  <div class="payOffUnDisbursement-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('payOffUnDisbursementTitle') }}
                  <v-spacer></v-spacer>
          ...
          style="align-items: center; margin-top: 0px !important;margin-bottom: 5px !important;border-top:1px solid black !important;margin-left: 25% !important;margin-right: 25% !important;">
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;padding: 0px !important;">
                        <span
                            style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{ $t('payOffUnDisbursementTitle') }}</span><br>
                </v-col>
...
export default {
  name: "PayOffUnDisbursementReport",
  meteor: {},
  mounted() {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("PayOffUnDisbursement");
      workbook.Sheets["PayOffUnDisbursement"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "PayOffUnDisbursement " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        this.dataHtml = "";
        this.dayMonthYear = this.FormatDate(this.params.dateAs);

        Meteor.call('sb_fetchPayOffUnDisbursementReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
