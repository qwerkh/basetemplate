          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('accountDueTitle') }}
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("AccountDue");
      workbook.Sheets["AccountDue"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "AccountDue " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        this.dataHtml = "";
        Meteor.call('sb_fetchAccountDueReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
