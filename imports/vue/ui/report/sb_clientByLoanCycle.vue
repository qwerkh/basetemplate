<!--suppress ALL -->
<template>
  <div class="clientByLoanCycle-report">
    <a4>
          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('clientByLoanCycleTitle') }}
                  <v-spacer></v-spacer>
                ...
                <v-card-text>
                  <v-row>
                    ...
                    <v-col cols="12" sm="4" md="4" class="d-flex">
                      <v-text-field v-model="params.loanCycle"
                                    outlined
                                    :dense="dense"
                                    type="number"
                                    :label="$t('loanCycle')"
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("ClientByLoanCycle");
      workbook.Sheets["ClientByLoanCycle"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "ClientByLoanCycle " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('sb_fetchClientByLoanCycleReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
