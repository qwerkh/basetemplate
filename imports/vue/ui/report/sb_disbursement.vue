          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('disbursementTitle') }}
                  <v-spacer></v-spacer>
                ...
                <v-card-text>
                  <v-row>
                    ...
                    <v-col cols="12" sm="4" md="4" class="d-flex" v-if="companyDoc.isSpecialLoan===true">
                    ...
                    <v-col cols="12" sm="4" md="4" class="d-flex" v-if="companyDoc.isPaymentType">
                      <v-autocomplete v-model="params.paymentTypeId"
                                      :items="paymentTypeOption"
                                      outlined
                                      :dense="dense"
                                      clearable
                                      item-text="label"
              ...
              <center v-if="companyDoc.isHasBanner">
                ...
                <thead style="margin-top: 5px">
                    <tr>
                       ...
                       <th>{{ $t('clientName') }}</th>
                       <th>{{ $t('productName') }}</th>
                       <th>{{ $t('disbursementDate') }}</th>
                       <th>{{ $t('maturityDate') }}</th>

                       <th>{{ $t('purpose') }}</th>
                       <th>{{ $t('loanAmount') }}</th>
                       <th>{{ $t('projectInterest') }}</th>
                       <th v-show="isOperationFee">{{ $t('projectOperation') }}</th>
                       <th>{{ $t('fee') }}</th>
                       <th>{{ $t('totalDue') }}</th>
                       <th v-if="companyDoc.isCostOfFund">{{ $t('costOfFund') }}</th>
                        <th>{{ $t('currency') }}</th>
                       <th>{{ $t('term') }}</th>
                       <th>{{ $t('rate') }}</th>
                       <th>{{ $t('cycle') }}</th>
                       <th>{{ $t('accType') }}</th>
                        ...
                        <th>{{ $t('address') }}</th>
                        <th v-if="companyDoc.isSpecialLoan">{{ $t('businessPartner') }}</th>
                        <th v-if="companyDoc.isSpecialLoan">{{ $t('phoneNumber') }}</th>
                        <th v-if="companyDoc.isSpecialLoan">{{ $t('company') }}</th>
                        <th v-if="companyDoc.isSpecialLoan">{{ $t('newLoanAmount') }}</th>
                        <th v-if="companyDoc.isSpecialLoan">{{ $t('newInterest') }}</th>
                        <th v-if="companyDoc.isSpecialLoan">{{ $t('newTerm') }}</th>
                    </tr>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("Disbursement");
      workbook.Sheets["Disbursement"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "Disbursement " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('sb_fetchDisbursementReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
