          ...
          <v-row type="flex" class="row-bg" justify="center">
            <v-col cols="12">
              <v-card
                  ...
                  max-width="100%"
              >
                <v-card-title>{{ $t('cutAccountTitle') }}
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      ...
      workbook.SheetNames.push("CutAccount");
      workbook.Sheets["CutAccount"] = worksheet;

      GlobalFn.exportExcelFile(workbook, "CutAccount " + moment().format("YYYYMMDD") + ".xlsx");
    },
    FormatNumber(val) {
    ...
    handleRun() {
      ...
      if (this.$refs.formReport.validate()) {
        ...
        Meteor.call('sb_fetchCutAccountReport', this.params, Constants.secret, vm.$i18n.locale, (err, result) => {
          if (result) {
